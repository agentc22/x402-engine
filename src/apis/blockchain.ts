...
router.post("/api/wallet/balances", async (req: Request, res: Response) => {
  ...
  try {
    ...
    res.status(502).json({ error: "Failed to fetch wallet balances", details: err.upstream || err.message });
  } finally {
    logRequest({
...
router.post("/api/wallet/transactions", async (req: Request, res: Response) => {
  ...
  try {
    ...
    upstreamStatus = err.status || 500;
    res.status(502).json({ error: "Failed to fetch transactions", details: err.upstream || err.message });
  } finally {
    logRequest({
...
router.post("/api/wallet/pnl", async (req: Request, res: Response) => {
  const { chain, address, min_liquidity, min_volume_24h } = req.body || {};
  ...
  try {
    ...
    upstreamStatus = err.status || 500;
    res.status(502).json({ error: "Failed to fetch wallet P&L", details: err.upstream || err.message });
  } finally {
    logRequest({
      service: "wallet-pnl",
...
router.post("/api/token/prices", async (req: Request, res: Response) => {
  ...
  try {
    ...
    upstreamStatus = err.status || 500;
    res.status(502).json({ error: "Failed to fetch token prices", details: err.upstream || err.message });
  } finally {
    logRequest({
...
router.get("/api/token/metadata", async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (req.query.chain) query.chain = String(req.query.chain);
    if (req.query.address) query.address = String(req.query.address);
    if (req.query.slug) query.slug = String(req.query.slug);
    ...
    upstreamStatus = err.status || 500;
    res.status(502).json({ error: "Failed to fetch token metadata", details: err.upstream || err.message });
  } finally {
    logRequest({
